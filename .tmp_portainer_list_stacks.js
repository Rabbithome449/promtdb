...
async function j(path,opts={}){const headers={...(opts.headers||{}),...cfHeaders};const r=await fetch(base+path,{...opts,headers});const t=await r.text();let d;try{d=JSON.parse(t)}catch{d=t}if(!r.ok)throw new Error(`${r.status} ${String(t).slice(0,200)}`);return d;}
...
const endpoints=await j('/api/endpoints',{headers});
for(const e of endpoints){
  try{const stacks=await j(`/api/stacks?endpointId=${e.Id}`,{headers});
    console.log('endpoint',e.Id,e.Name,'stacks',Array.isArray(stacks)?stacks.length:0);
    for(const s of (Array.isArray(stacks)?stacks:[])) console.log(`- id=${s.Id} name=${s.Name} type=${s.Type}`)
  }catch(err){console.log('endpoint',e.Id,e.Name,'error',String(err).slice(0,200))}
}
})();